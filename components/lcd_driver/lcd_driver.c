...
esp_err_t lcd_init(lcd_dev_t *dev, uint16_t width, uint16_t height)
{
    ...
    // Send AXS15231B QSPI init sequence directly
    uint8_t zero = 0x00;
    esp_lcd_panel_io_tx_param(io_handle, 0x11, &zero, 0);  // Sleep Out
    vTaskDelay(pdMS_TO_TICKS(120));
    esp_lcd_panel_io_tx_param(io_handle, 0x29, &zero, 0);  // Display ON
